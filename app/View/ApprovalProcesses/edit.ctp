...
<div class="approvalProcesses form col-md-12">
	...
	<div class="row">
		...
		echo "<div class='col-md-12'>".$this->Form->input('applicable_to',array('name'=>'data[ApprovalProcess][applicable_to][]', 'class'=>'form-control','multiple', 'value'=> json_decode($this->request->data['ApprovalProcess']['applicable_to'],true) , 'options'=>$customTables)) . '</div>'; 
		...
		<div class="col-md-12">
			...
			<div class=" table-responsive">
				<table class="table table-bordered table-condenced" id="apptble" style="min-width: 1800px;">
						...
						<?php foreach($this->request->data['ApprovalStep'] as $approvalStep){ ?>
							<tr class="colrows thiscol<?php echo $i?>">
								<td>
									...
									<?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.process_step',array('class'=>'form-control','label'=>false,'default'=>$approvalStep['process_step'],'required'=>$required)) ?></td>
								<td><?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.title',array('class'=>'form-control','label'=>false,'default'=>$approvalStep['title'],'required'=>$required)) ?></td>
								<td><?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.comments',array('rows'=>2, 'class'=>'form-control','default'=>$approvalStep['comments'],'label'=>false,'required'=>$required)) ?></td>
								
								<td><?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.send_to_reviwers',array('checkbox'=>'checkbox','default'=>$approvalStep['send_to_reviwers'], 'label'=>false,
								'onchange'=>'udpatecheck('.$i.',"reviewer",this);')) ?></td>
								
								<td><?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.send_to_approvers',array('checkbox'=>'checkbox','default'=>$approvalStep['send_to_approvers'], 'label'=>false,
								'onchange'=>'udpatecheck('.$i.',"publisher",this);')) ?></td>
								
								<td><?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.send_to_department_hod',array('checkbox'=>'form-control', 'default'=>$approvalStep['send_to_department_hod'], 'label'=>false,
								...
								<?php 
									if($approvalStep['approval_mode'] == 0)$val = $approvalStep['approval_mode'];
									else $val = 1;
								?>

								<td><?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.approval_mode',array('class'=>'','type'=>'radio', 'legend'=>false, 'options'=>array(0=>'View Only',1=>'Edit'),'default'=>$val)) ?></td>
								...
								?>							
								<td><?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.ignore_department',array('class'=>'','type'=>'radio', 'legend'=>false, 'options'=>array(0=>'Yes',1=>'No'),'default'=>$val));?></td>
								
								<?php 
									if($approvalStep['ignore_branch'] == 0)$val = $approvalStep['ignore_branch'];
									else $val = 1;
								?>
								<td><?php echo $this->Form->input('ApprovalStep.steps.'.$i.'.ignore_branch',array('class'=>'','type'=>'radio', 'legend'=>false, 'options'=>array(0=>'Yes',1=>'No'),'default'=>$val));?></td>
								<td>
									<?php echo $this->Form->postLink('<i class="fa fa-minus btn-remove-record"></i>',
					...
					<footer>
						<tr><td colspan="15" class="text-right"><?php echo $this->Html->link('Add','#',array('onClick'=>'addrows('.$i.')'))?></td></tr>
					</footer>
				</table>
			</div>
		</div>
...
<script>
    ...
    $().ready(function() {
        ...
        $('#ApprovalProcessEditForm').validate({        	
            rules: {
        ...
        $("#submit_id").click(function(){
            if($('#ApprovalProcessEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#ApprovalProcessEditForm').submit();
            }
