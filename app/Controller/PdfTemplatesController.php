...
class PdfTemplatesController extends AppController {
    ...
    public function add($custom_table_id = null,$qc_document_id = null){
        ...
        if ($this->request->is('post')) {
            ...
            if(file_exists($path . DS . 'template.docx') && $this->request['data']['PdfTemplate']['custom_table_id']){
                ...
                $html = $this->_convert_to_html($url,'docx','html',null,'Template',$this->request['data']['PdfTemplate']['custom_table_id']);                
                ...
                $html = $this->_convert_to_html($url,'docx','html',null,'Header','header');  
            ...
            if($this->request->params['pass'][0]){
                ...
                if($this->request->params['pass'][0] == 'HeaderTemplate'){
                    ...
                    $this->_convert_to_html($url,'html','docx',null,$customTable['CustomTable']['name'],$customTable['CustomTable']['id']);                
    ...
    public function _convert_to_html($url = null,$filetype = null,$outputtype = null, $password = null, $title = null,$record_id = null){
        ...
        if ($new_data === FALSE) {
            ...
            if($this->request->data['PdfTemplate']['html_cleanup'] == 0 && $outputtype == 'html'){
             ...
             $savepath = WWW_ROOT .'files' . DS . $this->Session->read('User.company_id')  . DS . 'pdf_template' . DS . $record_id;
            ...
            $file_for_save = WWW_ROOT .'files' . DS . $this->Session->read('User.company_id')  . DS . 'pdf_template' . DS . $record_id . DS .  'template.'.$outputtype;
    ...
    public function edit($id = null){
        ...
        if ($this->request->is('post')) {
            ...
            if(file_exists($path . DS . 'template.docx')){
                ...
                $html = $this->_convert_to_html($url,'docx','html',null,'Template',$this->request['data']['PdfTemplate']['id']);
                $html = str_replace('&quot;','"',$html);
        ...
        if ($data["status"] == 2) {
            ...
            if (file_get_contents($downloadUri) === FALSE) {
                ...
                if (file_put_contents($file_for_save, $new_data)) {
                    ...
                    $html = $this->_convert_to_html($url,'docx','html',null,'Template',$record_id);
                    ...
                    if($pdfTemplate['PdfTemplate']['html_cleanup'] == 0){
                        $html = $this->_html_cleanup($html);
                    }
    ...
    public function _write_html_file($filename = null, $content = null, $record_id = null){
        ...
        fclose($myfile);    
        $url = Router::url('/', true) . 'files/'.$this->Session->read('User.company_id').'/pdf_template/header/template.html';
        $key = $this->_generate_onlyoffice_key('template'. date('Ymdhis'));
        $html = str_replace('&quot;','"',$html);        
        $html = $this->_convert_to_html($url,'html','docx',null,'Header','header');        
